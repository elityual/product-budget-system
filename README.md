...
Para corrigir o erro `23502` em `orcamento.valor_total` no Supabase, execute `supabase/migrations/202609100002_budget_total.sql` após `202609100001_approval_commercial_terms.sql`. A migração mantém a coluna existente ou a cria como `numeric(18,2)`, recalcula registros antigos pela soma dos itens e a torna obrigatória. A RPC preenche o total na criação e edição calculando quantidade × preço dos itens recebidos, sem confiar no total enviado pelo navegador. Cada gravação aceita no máximo um orçamento novo. A aplicação remota dessa migração é manual.
...
Implementado no código: no menu de Orçamentos, Listar orçamentos aparece antes de Orçamentos aprovados pelo cliente. A listagem geral mostra a situação Pendente ou Aprovado pelo cliente; o submenu dedicado lista apenas os aprovados, com pesquisa e paginação. O botão Aprovar orçamento abre a seleção de um pendente, permite preencher condições comerciais opcionais e confirma o aceite do cliente. A criação e a edição de pendentes não exibem as condições; a edição de aprovados permite corrigir condições, cliente e validade, mas apresenta itens e total somente para consulta. Todos os autenticados não anônimos podem aprovar. A aprovação não representa pagamento.

A coluna `orcamento.aprovado` começa falsa. `atlas_approve_budget(expected_revision,budget_code,conditions)` usa a revisão global e grava condições e aprovação atomicamente. A assinatura anterior de dois argumentos permanece compatível e preserva condições existentes. Repetição, orçamento inexistente e acesso anônimo são rejeitados.
...
A aprovação é iniciada exclusivamente pelo botão do cabeçalho Aprovar orçamento. A janela permite pesquisar por cliente ou código e selecionar um pendente. Após a seleção, a pesquisa e as opções são substituídas por um resumo fixo do orçamento e os campos de pagamento, prazo de entrega, local de entrega e observações recebem foco. As condições ficam em um painel de preenchimento opcional: pagamento e prazo aparecem lado a lado no desktop, local e observações ocupam a largura total, e todos os campos se empilham no celular. A edição de aprovados usa o mesmo painel. Para escolher outro orçamento, cancele e abra a janela novamente. Falhas mantêm a seleção e o preenchimento. Sem pendentes, o botão fica desabilitado. A migração `202609100003_approved_budget_items.sql` impede alteração dos itens de um orçamento aprovado no Supabase; a mesma regra é aplicada no SQLite local.
