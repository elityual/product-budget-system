...
test('seletor de armazenamento abre, troca os campos e recupera a seleção salva', async ({ page }) => {
  ...
  await page.goto('/');
  await expect(page.locator('#company-setting')).toHaveCount(0);
  await expect(page.locator('#auth-panel')).not.toContainText('Nome da empresa');
...
test('modo local abre sem login e apresenta ferramentas de backup', async ({ page }) => {
  ...
  await expect(page.locator('#company-dialog-error')).toContainText('CNPJ');
  await page.locator('#company-form [name=nome]').fill('Empresa local de teste');
  await page.locator('#company-form [name=cnpj]').fill('11.222.333/0001-81');
  await page.locator('#company-form [name=endereco]').fill('Rua Local, 1');
  await page.locator('#company-form [name=telefone]').fill('(11) 3333-4444');
  await page.locator('#company-form [name=email]').fill('local@example.test');
  await page.getByRole('button', { name: 'Salvar e continuar', exact: true }).click();
  await expect(page.locator('#company-dialog')).toBeHidden();
