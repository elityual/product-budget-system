...
| ORC-008 | Edição | A edição identifica código e situação, agrupa cliente e validade e apresenta itens com produto, descrição, quantidade, valor unitário e subtotal, além do total destacado e ações reunidas no rodapé. Em pendentes, catálogo e itens ficam em painéis lado a lado no desktop e é possível trocar cliente, validade e itens; a data de emissão e os nomes e preços históricos dos itens já existentes são preservados. Em aprovados, as condições vêm antes dos itens, que ocupam toda a largura e ficam somente para consulta. No celular, painéis e dados são empilhados sem rolagem horizontal. | Implementado |
| ORC-009 | Exclusão | A exclusão pede confirmação e remove os itens vinculados na mesma gravação. | Implementado |
| ORC-010 | Aprovação | Orçamentos pendentes podem ser aprovados pelo botão do cabeçalho, após uma única seleção, preenchimento opcional das condições comerciais e confirmação explícita do aceite do cliente. Depois de selecionar, pesquisa e opções ficam ocultas e um resumo fixo identifica o orçamento; para trocar, é preciso cancelar e abrir a janela novamente. As condições usam painel responsivo com pagamento e prazo lado a lado no desktop, local e observações em largura total e uma coluna no celular. Condições e aprovação são persistidas atomicamente. A aprovação não significa pagamento e aparece como “Aprovado pelo cliente” na listagem geral. | Implementado |
