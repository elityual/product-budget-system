...
`supabase/install.sql` é o ponto de entrada para instalações novas. `scripts/build-sql.mjs` concatena uma lista explícita e ordenada de nove migrações, normaliza quebras de linha e preserva as transações de cada arquivo. O comando `npm run build:sql` regenera esse artefato versionado; não se deve editá-lo manualmente. A inclusão de uma nova migração para instalação limpa exige atualizar a lista e regenerar o SQL. `scripts/check.mjs` e `scripts/package-source.mjs` comparam seu conteúdo com a geração esperada e rejeitam divergências antes da distribuição. As migrações individuais permanecem destinadas à atualização de bancos existentes. O teste `tests/integration/install-supabase.test.js` executa o arquivo completo com Auth simulado em PGlite e verifica cadastro, orçamento, aprovação, histórico e rejeição atômica de produtos inativos.
...
A migração `202609110001_inactive_budget_products.sql` instala gatilhos usados pela substituição transacional da RPC. Antes da exclusão, eles registram temporariamente as quantidades persistidas; na reinserção, consultam o status de produto já atualizado pelo payload e rejeitam produtos inativos novos ou acima da quantidade anterior. O SQLite faz a mesma comparação antes de iniciar a gravação e ignora essa regra somente no caminho validado de restauração. `budget.js` exclui inativos do catálogo, enquanto `budget-flow.js` mantém os itens históricos no resumo, identifica o status e limita a quantidade ao valor salvo. Redução e remoção são permitidas; o próximo salvamento compara com a nova quantidade persistida.
